...
An in-tree self-host rewrite now lives under `compiler/`, but canonical syntax enforcement is still authoritative in the Rust frontend/tooling path today. The self-host stage0 pipeline currently runs `lex_json`, bridge-free `parse_json`, and bridge-free `typeck_json` in Draton while `build_json` still bridges through Rust host builtins, so the rewrite must keep matching the same canonical surface instead of reviving compatibility-form syntax.
