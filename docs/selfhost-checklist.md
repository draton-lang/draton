...
- `[x]` Statement-1 probing shows simple first-statement variants pass, but binary-expression first-statement variants preserve the crash
...
| Statement-1 variants | `python3 tools/probe_selfhost_stmt1_variants.py --stage1 /tmp/draton_s1` | `simple stmt1 variants pass, but stmt1 binary-expression variants preserve the crash` | The first statement participates via binary-expression parsing pressure, not merely by existing |
...
- `[x]` Confirm that statement-1 simple variants pass while binary-expression variants preserve the crash
...
- `[ ]` Explain why only statement-1 binary-expression variants preserve the crash while simple statement-1 variants do not
