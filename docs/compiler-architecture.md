...
- `compiler/driver/pipeline.dt` now owns the hidden bridge-free stage0 `typeck_json` payload, while `compiler/driver/typeck_stage.dt` remains the planned full self-host typechecker payload path with typed-body and `use_effect` serialization
