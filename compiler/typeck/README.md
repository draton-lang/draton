...
- `compiler/driver/pipeline.dt` owns the current bridge-free hidden stage0 `typeck_json` payload
- `compiler/driver/typeck_stage.dt` and `compiler/typeck/**` remain the planned full self-host stage0 typechecker path
- the hidden Rust stage0 command no longer routes `typeck` through `host_type_json`; Rust remains the broader oracle for semantic parity
