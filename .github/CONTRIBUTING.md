...
The repository uses three long-lived branches with a strict promotion path:

- `dev` is the active development branch and should receive ongoing coding, routine fixes, refactors, and feature work first.
- `unstable` receives code promoted from `dev` for wider validation, release-candidate testing, and stabilization work.
- `main` is the stable branch and should only receive code that is already proven stable.
...
- If a change is not stable enough for users, it is not ready for `main`.
- Use `unstable` to validate what may become stable, not as a long-term fork from normal development.
