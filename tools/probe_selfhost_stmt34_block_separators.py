#!/usr/bin/env python3
"""Probe how plain and annotated block separators affect stmt3/stmt4 crashes."""
...
def main() -> int:
    ...
    if (
        results["empty-block"] == 0
        and results["block-let"] != 0
        and results["block-expr"] != 0
        and results["block-if-empty"] != 0
        and results["unsafe-empty"] != 0
        and results["unsafe-let"] != 0
        and results["pointer-empty"] != 0
        and results["comptime-empty"] != 0
    ):
        print(
            "summary: only a plain empty block leaves the both-bad stmt3/stmt4 pair passing; "
            "non-empty blocks and annotated empty blocks still make it crash"
        )
        return 1

    print("summary: stmt3/stmt4 block-separator pattern changed")
    return 0
